...
then
    ...
    do
        ...
        isRunning=`ps -ef | grep postgres | grep "\-D" | grep -v grep | grep gp_role=dispatch | grep "$gp_ver"  | wc -l`
...
### Scenario#2, only list build/port/status ###
	...
	log_file=`find $COORDINATOR_DIRECTORY/log/ -size +0 | grep gpdb- | tail -1`; 
